...
4. Lisää kurssidata ajamalla `docker exec -i <KONTINNIMI> mongoimport -u root -p password --
authenticationDatabase=admin --drop -d events -c users --jsonArray < kurssit.json` komentorivillä ja korvaa <KONTINNIMI> docker kontin nimellä.
5. Lisää opiskelijadata ajamalla `docker exec -i <KONTINNIMI> mongoimport -u root -p password --
